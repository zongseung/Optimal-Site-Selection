...
> **정리 노트 (2026-08-11)**: 프로토타입 스크립트(`proto_v0_dose.py` · `proto_v0_fulln_cv.py` ·
> `proto_v1_ellipse.py` · `proto_v2_confirm.py` · `integrate_v2blend.py`)는 결론이
> `pfire/exposure_v2.py` 와 `scripts/run_phase1_mvp.py` 에 통합된 뒤 삭제했다(git 이력에 보존).
> 남은 코드는 보고서 그림 재생성용 `make_flagship_fig.py` 뿐이다.
...
프로토타입 `proto_v0_dose.py`(영동 부분집합) · `proto_v0_fulln_cv.py`(전 전주, 삭제됨) · `make_flagship_fig.py`.
...
`proto_v1_ellipse.py`(타원·국지정규화) · `proto_v2_confirm.py`(w스윕×5seed, 둘 다 삭제됨 — 로직은 `pfire/exposure_v2.py`). 산출 `outputs/pole_dose_v2_ellipse.parquet`, 그림 `fig_v1_corridor.png`.
...
**채택**: `R = 1−(1−R발화)(1−w·dose_v2.2)`. 파이프라인 `scripts/run_phase1_mvp.py` 에 병합 완료
(`config.EXPOSURE_V2_BLEND_W`; standalone 적용 스크립트 `integrate_v2blend.py` 는 병합 후 삭제).
