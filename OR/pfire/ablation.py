...
def build_risk(shared: dict, feature_set: str | tuple[str, ...],
    ...
    weights : numpy.ndarray, shape (R, d)
        체제별 행-simplex 가중(행합=1, R=len(config.REGIMES), d=feature_set 길이).
    ...
    n_reg = len(config.REGIMES)
    if w.ndim != 2 or w.shape[0] != n_reg or w.shape[1] != len(keys):
        raise ValueError(
            f"weights 형상 {w.shape} != ({n_reg}, {len(keys)}) (feature_set={keys})")
...
def _worker_eval(w_flat: np.ndarray) -> float:
    ...
    핵심 함정 회피: candidate 를 (R, n_feats) 로 reshape 할 때 tune_weights 의
    모듈전역 N_FEATS(=7) 가 아니라 _G["n_feats"] 를 쓴다 — 스폰된 자식이 기본 7 로
    misreshape 하는 것을 막는다.
    """
    d = int(_G["n_feats"])
    w = np.asarray(w_flat, dtype=np.float64).reshape(len(config.REGIMES), d)
...
def tune_for_featureset(
    ...
    -------
    dict
        weights : numpy.ndarray (R, d) — 튜닝된 행-simplex 가중.
    ...
    per_regime = [_sample_simplex_lhs(rng, n_random, d) for _ in config.REGIMES]
    cands = np.stack(per_regime, axis=1)           # (n,R,d)
