...
# ──────────────────────────────────────────────────────────────────────────
REGIME_YEONGDONG = "yeongdong"   # 영동 해안(강릉·동해·삼척): 가뭄·FWI driven (검증: 건조 AUC 0.727)
REGIME_CORRIDOR = "corridor"     # 양간지풍 회랑(양양·고성·속초): 바람 driven·송전선발화(2019 고성)
REGIME_YEONGSEO = "yeongseo"     # 영서 내륙: 농경 소각·입산자 실화(인간발화)
REGIME_MOUNTAIN = "mountain"     # 고지 산간: 연료·산림 지배, FWI 낮음

REGIMES = (REGIME_YEONGDONG, REGIME_CORRIDOR, REGIME_YEONGSEO, REGIME_MOUNTAIN)

# 풍하노출(양간지풍 서풍 prior·조건부 α_p)을 적용할 wind-driven 체제.
#   근거: 케이스-크로스오버 풍속 AUC — 회랑 0.664·영동잔여 0.562, 그 외 ~0.49(무변별).
FOEHN_REGIMES = (REGIME_YEONGDONG, REGIME_CORRIDOR)

SGG_TO_REGIME = {
    # 영동 해안(가뭄·FWI driven) — 회랑 제외
    "강릉": REGIME_YEONGDONG, "동해": REGIME_YEONGDONG, "삼척": REGIME_YEONGDONG,
    # 양간지풍 회랑(바람 driven)
    "속초": REGIME_CORRIDOR, "양양": REGIME_CORRIDOR, "고성": REGIME_CORRIDOR,
    # 영서 내륙
...
#   아래 주석에 보존.
# within-목적 튜닝(4레짐 joint, tuned_weights_within.json): 목적함수=레짐별 within recall
#   동일가중 평균(between-regime 기저율 지름길 제거). 전체 within-regime AUC 0.560(전역목적
#   0.488 대비↑), 회랑 0.42→0.52. soft 게이트가 전문가를 블렌딩하므로 4레짐을 함께
#   최적화한 값을 통째로 채택(부분 혼합 시 결합 파괴 — 하이브리드 실험서 0.437로 악화 확인).
#   trade-off: 전역 recall은 하락(top5 0.097→0.071) — per-pole 정확성·운영커버리지 우선 선택.
EXPERT_WEIGHTS = {
    # 영동: 가뭄/바람 driven(건조 AUC 0.727·FWI 0.725)과 정합.
    REGIME_YEONGDONG: dict(forest=0.0000, road=0.0000, powerline=0.0526, fwi=0.3158, yanggan=0.3684, fuel=0.2105, landcover=0.0526),
    # 회랑: 양간지풍 바람(yanggan) + 해안 토지피복. 고성2019 sanity 유지.
    REGIME_CORRIDOR:  dict(forest=0.0500, road=0.1500, powerline=0.0500, fwi=0.1000, yanggan=0.2500, fuel=0.0000, landcover=0.4000),
    # 영서: 연료·바람·토지피복(인적발화 — within 신호 약해 가중 해석 주의).
    REGIME_YEONGSEO:  dict(forest=0.0500, road=0.0000, powerline=0.0500, fwi=0.0500, yanggan=0.3500, fuel=0.3500, landcover=0.1500),
    # 산악: 도로 지배(고지 인적접근=도로변 입산자 발화 가설).
    REGIME_MOUNTAIN:  dict(forest=0.0000, road=0.7619, powerline=0.0000, fwi=0.0476, yanggan=0.0952, fuel=0.0476, landcover=0.0476),
}
...
# ──────────────────────────────────────────────────────────────────────────
# 풍하 노출 v2.2 (변별 가능 dose; pfire.exposure_v2). 근거·검증: exposure_v2/README.md
#   기존 OR 노출이 영동 포화(P≈1)라 변별 0 → 발화가중 기대도즈+LWR타원+국지정규화로
...
EXPOSURE_V2_LWR_CAP = 8.0         # Anderson 길이/폭비 상한
EXPOSURE_V2_NORM_CELL_KM = 4.0    # 국지정규화 격자 셀(km)
