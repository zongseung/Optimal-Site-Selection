...
for ax, (col, tt, mp) in zip(axes, [("risk_pctile", "Before: 3-regime", mp_old),
    ...
    ax.scatter([CX], [CY], marker="*", s=560, c="cyan", ec="black", lw=1.4, zorder=5,
               label="2019 고성 송전선 산불")
    ax.set_title(f"위험 백분위 (5km 평균 {mp:.3f})", fontsize=11)
    ax.set_xlabel("경도"); ax.legend(loc="lower left", fontsize=8)
axes[0].set_ylabel("위도")
cb = fig.colorbar(sc, ax=axes, shrink=.82, pad=.02); cb.set_label("전역 위험 백분위 (0-100)")
fig.suptitle("2019 고성 송전선 산불 — 위험 백분위",
             fontsize=12.5, y=.99)
