...
def fig_decision_map(sub: pl.DataFrame, admin_rings, pos: pl.DataFrame):
    """위험 판정(decision=1) 전주 + 과거 발화점★ 단독 지도.

    fig1_risk_map 의 (b)패널만 단독 1패널로 분리한 것. 백분위 그라데이션은
    시군별 위험지도(fig7)가 담당하므로, 전역 지도는 '판정+발화점'만 보인다.
    """
    ...
    if pos.height:
        ax.scatter(pos["lon"].to_numpy(), pos["lat"].to_numpy(), marker="*",
                   s=55, c="#1a1a1a", edgecolors="white", linewidths=0.4,
                   zorder=5, label="과거 발화점 (검증 앵커)")
    ax.set_title("강원 전주 산불 위험 판정 & 발화점",
                 fontsize=14, fontweight="bold")
    ax.set_xlabel("경도 (°E)")
    ax.set_ylabel("위도 (°N)")
    ax.legend(loc="lower right", fontsize=8, framealpha=0.9, markerscale=3)
    fig.tight_layout()
    _save(fig, "fig_decision_map.png")
...
def fig5_goseong(sub: pl.DataFrame, pos: pl.DataFrame):
    ...
    # 슬림 제출(risk_pctile 컬럼 없음)이면 risk_score의 국지 순위백분위로 대체(계단붕괴 회피).
    if "risk_pctile" in sub.columns:
        s_risk = sub["risk_pctile"].to_numpy()[m]
    else:
        _rs = sub["risk_score"].to_numpy()[m]
        _rank = np.argsort(np.argsort(_rs))
        s_risk = _rank / max(len(_rs) - 1, 1) * 100.0
...
def main():
    ...
    log.info("=== fig_decision 위험 판정 단독 지도 ===")
    fig_decision_map(sub, admin_rings, pos)
