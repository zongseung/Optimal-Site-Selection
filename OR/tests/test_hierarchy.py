...
def _synthetic_poles() -> pl.DataFrame:
    ...
    - 고성(g_cd_big, corridor, 대표본): 발화 다수 → corridor 체제 앵커 고정.
    - 속초(g_tiny_zero, corridor, 소표본): 발화 0 → 부모(corridor)율로 강축소.
    - 양양(g_tiny_one, corridor, 소표본): 발화 소수(1) → 부모로 강축소.
    ...
    # 대표본 corridor(체제 앵커 안정화)
    add("고성", "g_cd_big", 128.90, 37.75, 100_000)    # 발화 다수 예정
    # 소표본 corridor
...
def _fires_for(poles: pl.DataFrame, n_hot: int, n_tiny_one: int) -> pl.DataFrame:
    """big_hot·cd_big 격자에 발화점, tiny_one 격자에 n_tiny_one개 발화점 생성.

    big_hot(춘천)·cd_big(고성)에 n_hot개씩 둬 두 체제 모두 발화 신호가 있게 하고
    (corridor 체제 앵커 안정화), tiny_one(양양)에 소수만 둔다. 발화점 좌표는 해당
    격자 첫 전주 좌표에 미세 오프셋(<<1km)을 줘 최근접 귀속이 그 전주가 되게 한다.
    """
    rows = []
    for grid, cnt in (("g_big_hot", n_hot), ("g_cd_big", n_hot), ("g_tiny_one", n_tiny_one)):
...
def test_zero_fire_small_region_shrinks_to_parent():
    ...
    fires = _fires_for(poles, n_hot=200, n_tiny_one=1)
    tbl = hierarchy.region_rate_table(poles, fires)
    # 속초(tiny_zero, corridor, 발화0) → 부모 corridor율로 수렴.
...
def test_zero_fire_region_not_zero_multiplier():
    ...
    assert sok_mult.min() > 0.1  # 부모(corridor)율 수준 — 0 으로 붕괴하지 않음.
