...
def test_gate_rows_sum_to_one():
    ...
    assert w.shape == (m.height, len(config.REGIMES))
...
def test_ignition_candidates_top_quantile():
    ...
    n = 50_000
    I = rng.uniform(0, 1, n)
    k = len(config.REGIMES)
    gate = np.full((n, k), 1 / k)
...
def _gate_regimes(n: int, yd_frac: float, rng) -> tuple[np.ndarray, list[str]]:
    """영동 우세 yd_frac 비율, 나머지는 타 체제 우세인 합성 게이트(행합=1)."""
    order = list(config.REGIMES)
    k = len(order)
    yd_col = order.index(config.REGIME_YEONGDONG)
    gate = rng.uniform(0, 0.2, (n, k))
    ...
    for i in range(n):
        if is_yd[i]:
            ...
            other = [c for c in range(k) if c != yd_col]
...
def test_conditional_alpha_range_and_zero_outside_yeongdong():
    ...
    n = 1000
    gate, order = _gate_regimes(n, yd_frac=0.3, rng=rng)
...
def test_conditional_alpha_zero_base_disables():
    ...
    n = 500
    gate, order = _gate_regimes(n, yd_frac=0.5, rng=rng)
    yang = rng.uniform(0, 9, n)
...
def test_conditional_alpha_rejects_bad_base():
    ...
    gate, order = _gate_regimes(100, 0.3, rng)
...
def _regime_labels(n: int, seed: int = 0) -> np.ndarray:
    ...
    rng = np.random.default_rng(seed)
    k = len(config.REGIMES)
    w = np.arange(k, 0, -1, dtype=float)  # 불균형 분포(첫 체제 우세)
    return rng.choice(list(config.REGIMES), n, p=w / w.sum()).astype("<U16")
